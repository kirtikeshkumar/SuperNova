...
writefnameGe = "SNn_Counts_Livermore_Ge"+str(round(T_nu[ll],2))+"s-"+str(round(T_nu[hl-1],2))+"s_ERange_"+str(int(Enurangemin))+"-"+str(int(Enurangemax))+".dat"
print(writefnameGe)
...
def differentialRate_CEvNS(E_R, A, Z, Enu_min, Enu_max, gsq=0.0, m_med=1000.0):
...
##   E_R2=np.logspace(-3.0, np.log10(ER_max_Sapphire), 51)
   ...
   ##
   ##
   p1=lambda x:norm*diffRate_CEvNS(x, A_Si, Z_Si, Enu_min, Enu_max)
   p2=lambda x:norm*diffRate_CEvNS(x, A_Ge, Z_Ge, Enu_min, Enu_max)
   p3=lambda x:norm*(diffRate_CEvNS(x, A_Al, Z_Al, Enu_min, Enu_max)*f_Al+diffRate_CEvNS(x, A_O, Z_O, Enu_min, Enu_max)*f_O)
