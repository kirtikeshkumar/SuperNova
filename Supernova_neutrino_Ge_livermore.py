...
E_nu = np.linspace(0.00001,100,250)          ## Neutrino Energy
Enu_max = max(E_nu)
...
flux_net = (np.array(intFlux_nue) + np.array(intFlux_anue) + np.array(intFlux_nux)*4)*normalisation
neutrino_flux_list = InterpolatedUnivariateSpline(E_nu, flux_net, k = 1)

neutrino_flux_nakazato=np.vectorize(neutrino_flux_list)

pLe = pl.plot(E_nu,neutrino_flux_nakazato(E_nu),label=r'$\nu_e$',color='blue',linewidth=2)

pl.xlim(0,50)
pl.ylim(0,2e14)
pl.xlabel("neutrino energy [MeV]", fontsize=14)
pl.ylabel("total no. of neutrino [MeV$^{-1}$]", fontsize=14)
...
pl.text(250, 50, "Livermore", fontsize=35, weight='bold', ha='center', va='center', color='black',
