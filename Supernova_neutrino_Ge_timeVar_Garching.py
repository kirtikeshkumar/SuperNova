...
###########################################################################
folder_path = "data/Garching_Output/"  
times = np.loadtxt(folder_path+"garching_pinched_info_key.dat")
time_values = times[:,1]
############################################################################
ll = 0
hl = len(time_values)
...
for i in range(ll,hl):
   time = time_values[i]
   print(time," : Now Loading : "+folder_path+"pinched_"+str(i)+".dat" )
   data = np.loadtxt(folder_path+"pinched_"+str(i)+".dat") #GeV vs No./GeV
   ########################################################################
   ...
   neutrino_flux_list = InterpolatedUnivariateSpline(data[:,0]*1e3, (data[:,1]+data[:,4]+data[:,2]*4)*normalisation*1e-3, k = 1) #MeV vs 1/MeV/m^2
##   bins=np.append(data[:,0],data[-1,1])
##   flux=(data[:,2]+data[:,3]+data[:,4]*4)*normalisation
   Enu_min = np.max([np.min(data[:,0])*1000,0.0001])
   Enu_max = np.max(data[:,0])*1000
...
##   neutrino_flux_nakazato=np.vectorize(neutrino_flux_hist)
   neutrino_flux_garching=np.vectorize(neutrino_flux_list)

   ###Plot neutrino flux

##   pl.plot(data[:,0]*1e3,data[:,1]*1e-3,label=r'$\nu_e$',color='blue',linewidth=2)
##   pl.plot(data[:,0]*1e3,data[:,4]*1e-3,label=r'$\bar{\nu}_e$',color='green',linewidth=2)
##   pl.plot(data[:,0]*1e3,data[:,2]*1e-3,label=r'$\nu_x$',color='red',linewidth=2)
##   
##   
...
writefnameGe = "SNn_Counts_Ge"+str(time_values[ll])+"s-"+str(time_values[hl-1])+"s.dat"
wf = open("data/intp3003_Ge/"+writefnameGe,'wt')
line0 = "time \t\t mean eVee \t \t stdev eVee \t \t mean eVth \t \t stdev eVth \n"
line1 = " \t  \t 10 \t 100 \t 170 \t 10 \t 100 \t 170 \t 10 \t 100 \t 170 \t 10 \t 100 \t 170 \n"
wf.writelines(line0)
wf.writelines(line1)

for key in avgNumEvtGe10eVee.keys():
    line2 = str(key)+" \t "+str(avgNumEvtGe10eVee[key])+" \t "+str(avgNumEvtGe100eVee[key])+" \t "+str(avgNumEvtGe170eVee[key])+" \t "+str(stdevNumEvtGe10eVee[key])+" \t "+str(stdevNumEvtGe100eVee[key])+" \t "+str(stdevNumEvtGe170eVee[key])
...
writefnameSap = "SNn_Counts_Sap"+str(time_values[ll])+"s-"+str(time_values[hl-1])+"s.dat"
...
for key in avgNumEvtGe10eVee.keys():
    line2 = str(key)+" \t "+str(avgNumEvtSap50eVth[key])+" \t "+str(avgNumEvtSap100eVth[key])+" \t "+str(stdevNumEvtSap50eVth[key])+" \t "+str(stdevNumEvtSap100eVth[key])+"\n"
    wf.writelines(line2)
wf.close()
