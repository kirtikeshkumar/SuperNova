...
effsapdat=np.loadtxt("EfficiencySapphire.dat") ## in keV vs probability
effsapinterp = InterpolatedUnivariateSpline(effsapdat[:,0],effsapdat[:,1],k=1)
effGe10dat=np.loadtxt("EfficiencyGe_10eV_run2.dat") ## in keV vs probability
effGe10interp = InterpolatedUnivariateSpline(effGe10dat[:,0],effGe10dat[:,1],k=1)
effGe100dat=np.loadtxt("EfficiencyGe_100eV_run2.dat") ## in keV vs probability
effGe100interp = InterpolatedUnivariateSpline(effGe100dat[:,0],effGe100dat[:,1],k=1)


E_Ree_Ge = EH(E_R1)
E_Ree_Ge_mid = 0.5*(E_Ree_Ge[1:]+E_Ree_Ge[:-1])
E_R_Sap_mid = 0.5*(E_R2[1:]+E_R2[:-1])

CountGe10effmult = []
CountGe100effmult = []
CountSapeffmult = []
for i in range(len(E_Ree_Ge_mid)):
   CountGe10effmult.append(CountGe[i]*effGe10interp(E_Ree_Ge_mid[i]))
   CountGe100effmult.append(CountGe[i]*effGe100interp(E_Ree_Ge_mid[i]))
   CountSapeffmult.append(CountAl2O3[i]*effsapinterp(E_R_Sap_mid[i]))
##   print("efficiency at ",E_Ree_Ge_mid[i]," of Ge10 ",effGe10interp(E_Ree_Ge_mid[i])," of Ge100 ",effGe100interp(E_Ree_Ge_mid[i]))
##   print("efficiency at ",E_R_Sap_mid[i]," of Sap ",effsapinterp(E_R_Sap_mid[i]))
   
print("number of events in Sapphire with Threshold 100eV is:", sum(CountSapeffmult))
print("number of events in Ge with Threshold 10eV is:", sum(CountGe10effmult))
print("number of events in Ge with Threshold 100eV is:", sum(CountGe100effmult))
