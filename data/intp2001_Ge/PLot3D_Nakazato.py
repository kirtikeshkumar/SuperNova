...
massinCode = 100 # in kg, the mass for which the 3D code was run
masstoeval = 10  # in kg, the mass for which to plot
...
############################################################################
files = os.listdir("./Nakazato3D/")

f = pd.read_csv("./Nakazato3D/"+files[0],delimiter='	',header=None)
ErecBins = np.array(f.iloc[0][:-1])
ErecMid = 0.5*(ErecBins[1:]+ErecBins[:-1])
binwdt = ErecBins[1:]-ErecBins[:-1]
shp = f.shape
...
##    print(file_name,'\t',lowtime,hitime,'\t',lowE,hiE,'\t',loTIndex,hiTIndex,'\t',histname[0])
    ...
    f = pd.read_csv("./Nakazato3D/"+file_name,delimiter='	',header=None)
    shp = f.shape
    f = np.array(f.iloc[1:shp[0],0:shp[1]-2])
    globals()[histname[0]][loTIndex:hiTIndex+1] = f/massinCode*masstoeval/binwdt ## the (*masstoeval/massinCode) evaluates result for required detector mass
                                                                                 ## the division by binwdt gives result in /keV.
...
ax.set_zlabel('log10(Counts/10kg/keV/s)')
##ax.set(zscale='log')
...
np.savetxt('Nakazato3D_gridpoints/gridpoints_0_15.txt', gridpoints_0_15, delimiter='\t', fmt='%.6e')
np.savetxt('Nakazato3D_gridpoints/gridpoints_15_30.txt', gridpoints_15_30, delimiter='\t', fmt='%.6e')
np.savetxt('Nakazato3D_gridpoints/gridpoints_30_45.txt', gridpoints_30_45, delimiter='\t', fmt='%.6e')
np.savetxt('Nakazato3D_gridpoints/gridpoints_45_300.txt', gridpoints_45_300, delimiter='\t', fmt='%.6e')
