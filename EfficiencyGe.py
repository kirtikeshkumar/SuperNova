...
#data = np.loadtxt("supernova-spectrum_M_20_Z_2_rev_200.txt") #MeV vs No./MeV
...
##print(1)
##
ER_max_Ge = 1.0 ##upper cutoff in keV taken since 4 order of magnitude reduction in evt rate
ER_max_Sapphire = 1.0
##E_R1=np.linspace(0.001,ER_max_Ge,501)
E_R1 = np.logspace(-3.0, np.log10(ER_max_Ge), 2001) ## bin the recoil energies upto upper cutoff ER_max
##E_R2=np.linspace(0.001,ER_max_Sapphire,501)
E_R2 = np.logspace(-3.0, np.log10(ER_max_Sapphire), 2001)
#E_R1=np.linspace(0.0, ER_max, 201)
...
for i in range(0,len(E_R1)-1):
    print(i)
    CountGe[i]=10000.0*(-E_R1[i]+E_R1[i+1])##quad(p2,E_R1[i],E_R1[i+1],epsrel=1e-4)[0]
    #CountSi[i]=quad(p1,E_R1[i],E_R1[i+1],epsrel=1e-4)[0]
    CountAl2O3[i]=10000.0*(-E_R1[i]+E_R1[i+1])##quad(p3,E_R2[i],E_R2[i+1],epsrel=1e-4)[0]
...
ESigGe = np.vectorize(signormEheat)

QuenchedEGe = EH(E_R1)
SigGe = ESigGe(E_R1)

pl.plot(E_R1,QuenchedEGe,linewidth=2)
pl.yticks(fontsize=20)
pl.xticks(fontsize=20)
pl.xlim(0.001,ER_max_Ge)
pl.xlabel("Recoil Energy ($keV_{nr}$)",fontsize=24)
pl.ylabel("Electron Equivalent Energy (keVee)",fontsize=24)
##pl.legend( fontsize=18)
pl.show()

pl.plot(QuenchedEGe,SigGe,linewidth=2)
pl.yticks(fontsize=20)
pl.xticks(fontsize=20)
pl.xlim(0.001,ER_max_Ge)
pl.xlabel("Energy (keVee)",fontsize=24)
pl.ylabel("Resolution (keVee)",fontsize=24)
##pl.legend( fontsize=18)
pl.show()
...
SapSigE = np.vectorize(sigESapphire)

SigESap = SapSigE(E_R2)
pl.plot(E_R2,SigESap,linewidth=2)
pl.yticks(fontsize=20)
pl.xticks(fontsize=20)
pl.xlim(0.001,ER_max_Sapphire)
pl.xlabel("Recoil Energy (keV)",fontsize=24)
pl.ylabel("Resolution (keV)",fontsize=24)
##pl.legend( fontsize=18)
pl.show()
...
for run in range(numruns):
    ...
    for i in range(len(evtErecAl2O3)):
        evtenergiessapphire.append([evtErecAl2O3_noRes[i],evtErecAl2O3[i]])
    ...
    #             evtenergiessapphire.append([er,evtErecAl2O3[i]])
...
##    pl.show()
    ...
    for evt in evtErecGe:
        if(np.random.rand()<0.75):
            ...
            netEheat = heatnet(evt)
            evtenergies.append([evt,normEheat,netEheat])
            ...
            Eveto=np.append(Eveto,np.random.normal(evet,sigEveto))
            normEheat=np.random.normal(heatnorm(evt),signormEheat(evt))
            ##print(evt,heatnorm(evt),signormEheat(evt),normEheat)
            evtnormEheat=np.append(evtnormEheat,normEheat)
...
##    print("number of events in Ge with normalised recoil energy > 050eV is: ", numevt50eV[run])
##    print("number of events in Ge with normalised recoil energy > 100eV is: ", numevt100eV[run])
    
transposeevt=np.transpose(evtenergies)
allevt=transposeevt[0]
accevt=allevt[transposeevt[2]>0.03]
accevt100=allevt[transposeevt[2]>0.4]
accevt50=allevt[transposeevt[2]>0.2]
##binsevt=np.linspace(0.000,ER_max_Ge,251)
binsevt=np.linspace(0.0005,1,1000)
##binsevt=np.logspace(-3,np.log10(ER_max_Ge),2001)
allhist=pl.hist(allevt,bins=binsevt)[0]
acchist=pl.hist(accevt,bins=binsevt)[0]
acchist100=pl.hist(accevt100,bins=binsevt)[0]
acchist50=pl.hist(accevt50,bins=binsevt)[0]
pl.show()

transposeevtsap=np.transpose(evtenergiessapphire)
allevtsap=transposeevtsap[0]
accevtsap=allevtsap[transposeevtsap[1]>0.1]
##binsevtsap=np.linspace(0.000,ER_max_Sapphire,251)
binsevtsap=np.linspace(0.0005,1,750)
allhistsap=pl.hist(allevtsap,bins=binsevtsap)[0]
acchistsap=pl.hist(accevtsap,bins=binsevtsap)[0]
pl.show()

##pl.xscale('log')
##effevt=pl.hist(binsevt[:-1], binsevt, weights=acchist/allhist,histtype='step', label=r'Ge 10eVee', color='blue',linewidth=2)
##effevt50=pl.hist(binsevt[:-1], binsevt, weights=acchist50/allhist,histtype='step', label=r'Ge 50eVee', color='green',linewidth=2)
##effevt100=pl.hist(binsevt[:-1], binsevt, weights=acchist100/allhist,histtype='step', label=r'Ge 100eVee', color='black',linewidth=2)
##effevtsap=pl.hist(binsevtsap[:-1], binsevtsap, weights=acchistsap/allhistsap,histtype='step',label=r'Sapphire 100eVnr', color='red',linewidth=2)
##pl.yticks(fontsize=20)
##pl.xticks(fontsize=20)
##pl.xlim(0.001,ER_max_Sapphire)
##pl.xlabel("Recoil Energy (keV)",fontsize=24)
##pl.ylabel("Efficiency",fontsize=24)
##pl.legend( fontsize=18)
##pl.show()
...
def inverse_transform_x(x):
    return x*x/heatnorm(x)
fig, ax = pl.subplots()

effevt = ax.hist(binsevt[:-1], binsevt, weights=acchist/allhist, histtype='step', label=r'Ge $10eV_t$', color='blue', linewidth=2)
effevt50 = ax.hist(binsevt[:-1], binsevt, weights=acchist50/allhist, histtype='step', label=r'Ge $50eV_t$', color='green', linewidth=2)
effevt100 = ax.hist(binsevt[:-1], binsevt, weights=acchist100/allhist, histtype='step', label=r'Ge $100eV_t$', color='black', linewidth=2)
effevtsap = ax.hist(binsevtsap[:-1], binsevtsap, weights=acchistsap/allhistsap, histtype='step', label=r'Sapphire $100eV_{nr}$', color='red', linewidth=2)

# Add labels and limits
ax.set_xlabel("Recoil Energy ($keV_nr$)", fontsize=24)
ax.set_ylabel("Efficiency", fontsize=24)
ax.set_xlim(0.001, ER_max_Sapphire)
ax.tick_params(axis='both', which='major', labelsize=20)

# Create secondary x-axis at the top with the transformation
ax_top = ax.secondary_xaxis('top', functions=(transform_x, inverse_transform_x))
ax_top.set_xlabel("Recoil Energy ($keV_ee$)", fontsize=24)  # Modify label as needed
...
midbinsGe = 0.5*(binsevt[1:]+binsevt[:-1])
midbinsSap = 0.5*(binsevtsap[1:]+binsevtsap[:-1])

np.savetxt("Ge_10eVt_Efficiency.dat",np.transpose([midbinsGe,effevt[0]]))
np.savetxt("Ge_50eVt_Efficiency.dat",np.transpose([midbinsGe,effevt50[0]]))
##np.savetxt("Ge_100eV_Efficiency.dat",np.transpose([midbinsGe,effevt100[0]]))
np.savetxt("Sapphire_100eVt_Efficiency.dat",np.transpose([midbinsSap,effevtsap[0]]))
