...
ER_max_Sapphire = 1.0
E_R1=np.linspace(0.001,ER_max_Ge,51)##np.logspace(-3.0, np.log10(ER_max_Ge), 51) ## bin the recoil energies upto upper cutoff ER_max
E_R2=np.linspace(0.001,ER_max_Sapphire,51)##np.logspace(-3.0, np.log10(ER_max_Sapphire), 51)
...
##    pl.show()
    ...
    for evt in evtErecGe:
        if(np.random.rand()<0.75):
            ...
            evtenergies.append([evt,normEheat])
...
accevt=allevt[transposeevt[1]>0.01]
accevt100=allevt[transposeevt[1]>0.1]
...
pl.xscale('log')
effevt=pl.hist(binsevt[:-1], binsevt, weights=acchist/allhist,histtype='step', label=r'Ge', color='blue',linewidth=2)
effevt100=pl.hist(binsevt[:-1], binsevt, weights=acchist100/allhist,histtype='step', label=r'Ge', color='green',linewidth=2)
effevtsap=pl.hist(binsevtsap[:-1], binsevtsap, weights=acchistsap/allhistsap,histtype='step',label=r'Sapphire', color='red',linewidth=2)
pl.yticks(fontsize=20)
pl.xticks(fontsize=20)
pl.xlim(0.001,ER_max_Sapphire)
pl.xlabel("Recoil Energy (keV)",fontsize=24)
...
np.savetxt("Ge_10eV_Efficiency.dat",np.transpose([midbinsGe,effevt[0]]))
np.savetxt("Ge_100eV_Efficiency.dat",np.transpose([midbinsGe,effevt100[0]]))
np.savetxt("Sapphire_100eV_Efficiency.dat",np.transpose([midbinsSap,effevtsap[0]]))
